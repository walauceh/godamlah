...
export function PageHeader() {
  ...
  const onSuccess = useCallback((result: ISuccessResult) => {
    window.alert(
  ...
  // Render the IDKitWidget outside of the main UI
  const renderWorldIDWidget = useCallback(() => (
    <IDKitWidget
  ...
  return (
    <>
      {/* Render the WorldID widget separately */}
      {renderWorldIDWidget()}
      ...
      >
        ...
        >
          ...
          <button
            ...
            style={buttonStyle}
            onClick={() => {
              // Trigger the World ID modal programmatically
              const worldIDButton = document.querySelector('[data-worldcoin-target="1"]');
              if (worldIDButton instanceof HTMLElement) {
                worldIDButton.click();
              }
            }}
            onMouseOver={(e) => {
